...
const prompt = `
You are a helpful tool that can parse recipes from text and identify the ingredients and the steps present in the recipe. When presented with a recipe, you should return JSON consisting of three properties. The first should be of key "items", which should be an array of strings representing the ingredients in the recipe along with their amounts. You should take this directly from the recipe input. The second should be key "process", and this should be an array of strings representing the steps in the recipe. This should be taken directly from the recipe input with no changes.
...
export const actions = {
  parse: async ({ request }) => {
    ...
    if (!recipe) {
      return { action: 'parse', recipe: null, error: 'No recipe was provided.' };
    }
    ...
    try {
      ...
      return { action: 'parse', recipe: recipeJson };
    } catch (e) {
      return {
        action: 'parse',
        recipe: null,
        error: 'There was an issue parsing your recipe. Try inputting it manually.',
      };
    }
  },
};
