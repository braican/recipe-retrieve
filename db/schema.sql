...
create policy "Users can only view their flags." on flags
  for select using (auth.uid() = user_id);

create policy "Users can only insert their own flags." on flags
  for insert with check (auth.uid() = user_id);

create policy "Users can only update their own flags." on flags
  for update using (auth.uid() = user_id);

create policy "Users can only delete their own flags." on flags
